...
<?php
   /* $data=array("table"=>"db_theem","val"=>array("`id`, `user_id`, `user_store_id`, `theam_id`, `file_name`, `add_date`, `update_date`"),"where"=>array("`user_id`='111'"));
             $log=$this->ModelSite->getdata($data);
             print_r($log);
             exit();*/
    ?>
      ...
      ul#aboutus_sortable li, ul#aboutus_sortable2 li 
      {
       ...
       ul#aboutus_sortable li img, 
       ul#aboutus_sortable2 li img 
       { 
           //width: 100%;
           max-height: 50px;
       }
...
<div  id="<?php echo $pp_name;?>" state="disabled notRotatable" class="<?php echo $daynamiceresponcivepopup;?>"  >
        ...
        <div class="row">
            <?php echo topheaderpopupsaveclose('d_off("#'.$pp_name.'")','changein_menu()','Change About-us Image');?>
...
<!--                 <div class="col-lg-12" style="background-color: #FFFFFF;">                
                ...
                <div class="col-md-12 wrapaboutus_sortable1">                    
                     <?php $query="SELECT `id`, `image_path`, `view` FROM `userProd_image` WHERE   `status`='1' AND `about_us_pic`='1' AND  `user_id`='". $user_session_id."' order by `view`";
                        ...
                        ?>
                    <ul id="aboutus_sortable" data-toggle="popover" >
                   <?php while ($res_query=mysql_fetch_assoc($row_query))
                        {$count_nomer_ofmenu++;?>
                        <li  class="aboutus_sortable_li" id="<?php echo $res_query['id'];?>" value="<?php echo $res_query['view'];?>" >
...
<!--              <div class="col-lg-12" style="background-color: #FFFFFF;">                
                ...
                <div class="col-md-12 wrapaboutus_sortable1" >  
                    ...
                    <ul id="aboutus_sortable2" data-toggle="popover">
                        <?php                          
                        while ($res_query_c=mysql_fetch_assoc($row_query_c))
                        {
                      ?>                
                        <li  class="aboutus_sortable2_li" id="<?php echo $res_query_c['id'];?>" value="<?php echo $res_query_c['view'];?>">
            ...
            <div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12" >
                ...
                <div class=" col-lg-5 col-md-5 col-sm-6 col-xs-12 pull-right" >
                    ...
                    {
                        var data='<li  class="aboutus_sortable2_li" id='+id+' value="0"><img class="img img-responsive center-block block" src='+path+' alt="gallery" ></li>';
                        $('#aboutus_sortable2').append(data);
                        //alert(path);
                    }
                    </script>
                </div>
   ...
   $("#aboutus_sortable >li").popover({
    ...
    $("#aboutus_sortable2 >li").popover({
        ...
        $(document).ready(function () {
            $("#aboutus_sortable>li,#aboutus_sortable2>li").dblclick(function () {
...
<script type="text/javascript" >
  ...
  $(<?php echo '"#'.$pp_name.'"';?>).css("cursor", "move");  
    ...
    {
        if($('#aboutus_sortable li').length<1||$('#aboutus_sortable li').length>1)
                    {
                        alert('Please select only one About us image');       
                    }
                    else
                    { //alert($("#aboutus_sortable >li").attr("id"));
                        var data_id=$("#aboutus_sortable >li").attr("id");
                        //alert($('.aboutus_sortable_li img').attr('src'));
                       $('.user_detaii_image img').attr('src',$('.aboutus_sortable2_li img').attr('src'));
                       
                       // alert($('.aboutus_sortable2_li img').attr('src'));
                         $.ajax({
                            type: "POST",
                            url: "<?php echo BASE_URL."Inserdata/uboutus_image";?>",
                            async: false,
                            data: { id:data_id}
