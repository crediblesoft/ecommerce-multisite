...
class App extends MY_Controller
{
    ...
    {  
		...
		if($email_count==0 && $username_count==0) {  
			$data=array('table'=>'user_Info','val'=>array('f_name'=>$fname,'l_name'=>$lname,'email_id'=>$email,'mobile_no'=>$mobile,'address1'=>$address,'state'=>$state,'username'=>$username,'password'=> md5($password),'pass2'=>$password,'verify'=>$verify,'type_Of_User'=>$typeofuser,'store_info'=>"$store_info",'paid'=>"$paiduser"));                
	 ...
	 function updateprofilepic(){
	   ...
	   if(!empty($img))
		{	
			...
			$height = 120;
					// Get new dimensions
					list($width_orig, $height_orig) = getimagesize('assets/image/user/'.$image);
			...
			$image1 = imagecreatefromjpeg('assets/image/user/'.$image);
			imagecopyresampled($image_p, $image1, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);

			// Output
			$thumbimg=imagejpeg($image_p, null, 100);
	...
	function uploadproductpic(){
		...
		if(!empty($img))
		{
			...
			unlink('assets/image/product/thumb/'.$image);
			echo 'error';
			}	
