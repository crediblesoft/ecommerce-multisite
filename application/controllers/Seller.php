...
class Seller extends Editpenalcontroller{
        ...
        if($record['res']>0)
        {
            ...
            $themequery="SELECT `id`, `user_id`, `user_store_id`, `theam_id`, `file_name`, `add_date`, `update_date` FROM `db_theem` WHERE `user_id`='".$record['rows']->id."'";
                ...
                /*<?php $theme_id='info';//info ,success, warning ,danger ,primary,primary?>*/
                    ...
                    $data['events']=$this->modelsite->get_page_text($theme_user_id,'events');
            ...
            {    
                ...
                $mypageneme=array("prod_detail","prod_cart","about_us","prod_view","term_condition","gallery","contact_us","user_profile","events","1");
                ...
                if(in_array($pagename,$mypageneme))
                {
                   ...
                   switch ($pagename):
                            ...
                            case 'user_profile':
                                ...
                                break;
                            case 'events':
                                seller::edit_events($data,$theme_user_id);  
                                break;
                            case '1':
                                //seller::edit_1($data); 
                                break;
    ...
    public function edit_prod_view($data,$theme_user_id)
    {
        ...
        //echo "<pre>";
        //print_r($data["user_product"]);exit;
    ...
    public function edit_user_profile($data)
         ...
         public function edit_events($data,$theme_user_id)
            {//echo "hi";
            //echo $user_id; exit; 
           //  echo $username; exit;
           // echo $theme_user_id; exit;
             //print_r($data); exit;
             //print_r($this);exit;
             
                $table=Editpenalcontroller::viewsellerwiseEvents($theme_user_id);
                //print_r($data['seller_info']); exit;
                // echo "<pre>";
                //echo "dfsdf";
                //print_r($table); exit;
                $data['mainpagetitle']=$data['mainpagetitle13'];
                $data['events']=$table;
                $this->load->view('include/edit_panel/load_fileHead.php',$data);
                $this->load->view('include/edit_panel/header.php',$data);
                $this->load->view('custum_theme/events',$data);            
                $this->load->view('include/edit_panel/footer.php',$data);
            }         
    ...
    public function showeliment()
            ...
            public function menu($user_id,$user_name)
        {
            ...
            {     
                $aa=$aa."<li class='".$data->link."' id='".$data->id."'><a href='".BASE_URL.$user_name."/Shope/".$data->link."' > <span>".$data->label." </span></a>"; 
