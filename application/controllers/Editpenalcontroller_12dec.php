...
class Editpenalcontroller extends CI_Controller {
    ...
    public function addtocart($prodId,$qty=1){
        ...
        //echo $prodId."<br/>";
        $comment1=array('val'=>'p.id as prod_id,p.*,cat.category,cat.id as catid','table'=>'product as p','where'=>array("p.id"=>$prodId),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'p.id','orderas'=>'DESC');
        ...
        $ardata=array("table"=>"user_Info","where"=>array("id"=>$product->user_id),"val"=>array("username","f_name","l_name","id"));
        $record=$this->editmodel->getsinglerow($ardata);
        $user_name=$record['rows']->username;
        $seller_id=$record['rows']->id;
        $f_name=$record['rows']->f_name;
        $l_name=$record['rows']->l_name;
        //print_r($record);exit;    
        ...
        'options' => array('Category' => $product->category,'image'=> $product->prod_img,'sellerid'=>$seller_id, 'sellername' => $f_name.' '.$l_name)
        );
