...
class Recipe extends MY_Controller {
    ...
    {
        $userid=$this->session->userdata('user_id');        
        $comment1=array('table'=>'user_Info as u','val'=>'u.store_info,u.id,u.username,u.f_name,u.l_name,u.profile_Pic,rs.recipe_type, rs.recipe_title, rs.recipe_detail, rs.recipe_addDate, rs.recipe_updateDate,rs.image_path,rs.id as recipeid, rs.recipe_stetus,rs.admin_status,cat.category','where'=>array("u.verified"=>'1',"u.id"=>$userid),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'rs.id','orderas'=>'DESC');
        $multijoin1=array(
    ...
    {
        $this->functions->_valid_user();
        $this->check_img_size("recipe/updaterecipe");
        //$this->form_validation->set_rules('video_link','Video Link','trim|required');
        $this->form_validation->set_rules('name','Recipe Name','trim|required');
        $this->form_validation->set_rules('Type','Recipe Type','trim|required');
        $this->form_validation->set_rules('details','Recipe Details','trim|required');
				...
				if($retcode==200) 
					{
						...
						$this->session->set_flashdata("warning","Given image path is invalid. Try again with other path.");
                        redirect("recipe/editrecipe/".$recipeId,"refresh");  
					}
				//echo $retcode;				
				curl_close($ch);
				//echo 'tata';
            ...
            if($log)
                {
                    ...
                    $this->session->set_flashdata("warning","Recipe not updated");
                    redirect("recipe/recipeuserlist","refresh");                   
                }
            }else{ 
            $this->load->view('include/header');
