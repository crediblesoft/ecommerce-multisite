...
class Mail extends MY_Controller {
    ...
    public function trash(){
        ...
        $comment1=array('val'=>'m.id as mail_id,m.subject,m.message,m.timestamp,u.f_name,u.l_name','table'=>'mail_to as mt','where'=>array("mt.mail_to"=>$this->userid,"mt.status"=>"2"),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'m.id','orderas'=>'DESC');
        $multijoin1=array(  
            array('table'=>'mail as m','on'=>'mt.mail_from=m.id','join_type'=>''),
            array('table'=>'user_Info as u','on'=>'m.mail_from=u.id','join_type'=>''),
        );
        
        $comment2=array('val'=>'m.id as mail_id,m.subject,m.message,m.timestamp,u.f_name,u.l_name','table'=>'mail as m','where'=>array("m.mail_from"=>$this->userid,"m.status"=>"2"),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'m.id','orderas'=>'DESC');
        $multijoin2=array(
            array('table'=>'user_Info as u','on'=>'m.mail_from=u.id','join_type'=>''),
        );
        //$log['forumlist']=$this->common->multijoin($comment1,$multijoin1);
    ...
    public function search_inbox($users=array(),$user){
        ...
        $config = array();
        $config["base_url"] = BASE_URL. "mail/search?page=inbox&search=".$user;
        $config["total_rows"] = ($table['res'])?count($table['rows']):0;
        $config["per_page"] = 20;
