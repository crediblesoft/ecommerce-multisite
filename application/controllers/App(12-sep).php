...
//                'size_units' => 'in', // in (default), feet, cm
	...
	function get_code_details()
	{
	 ...
	 $uid=$this->input->post('uid');  
         //echo $uid;
         $comment1=array('table'=>'user_Info as u','val'=>'udf.status,udf.date,p.id as prodid,p.prod_img,p.prod_name,udf.buyerId,udf.price,udf.quantity,udf.trans_id,u.store_info,u.id,u.username,u.f_name,u.l_name,u.profile_Pic,ts.tax','where'=>array("u.id"=>$uid),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'udf.id','orderas'=>'DESC');
         $multijoin1=array(
        ...
        $comment=array('table'=>'user_Info as u','val'=>'up.tranc_id, up.price as themeprice, up.add_date,u.store_info,u.id,u.username,u.f_name,u.l_name,u.profile_Pic','where'=>array("u.id"=>$uid),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'up.id','orderas'=>'DESC');//
        $multijoin=array(            
