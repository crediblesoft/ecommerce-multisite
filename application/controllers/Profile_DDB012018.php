...
class Profile extends MY_Controller {
    ...
    public function storeinfo(){
        ...
        if($this->form_validation->run()){
            ...
            $data=array('table'=>'user_store_info','val'=>array('user_id'=>$this->userid,'business_name'=>$business_name,'contact_person_name'=>$contact_person,'phone'=>$phone,'address'=> $address,'zip'=>$zip,'lat'=>$getlanglat->lat,'lng'=>$getlanglat->lng,"size"=>$farmsize,"income"=>$income,'certification'=>$certification));                
...
//                );
    ...
    public function update(){ 
        ...
        if($basic_info){
            ...
            if($this->form_validation->run()){
                ...
                $updatedata=array('table'=>'user_Info','where'=>array('id'=>$this->userid),'val'=>array('f_name'=>$f_name,'l_name'=>$l_name,'mobile_no'=>$mobile,'state'=>$state,'address1'=>$address1));
                $updatelog=$this->common->update_data($updatedata);
		...
		if($income==''){$income=0;}
            ...
            $data=array('table'=>'user_store_info',"where"=>array("user_id"=>$this->userid),'val'=>array('user_id'=>$this->userid,'business_name'=>$business_name,'contact_person_name'=>$contact_person,'phone'=>$phone,'address'=> $address,'zip'=>$zip,'lat'=>$getlanglat->lat,'lng'=>$getlanglat->lng,"size"=>$farmsize,"income"=>$income,'certification'=>$certification));                
            $log=$this->common->update_data($data);
    ...
    public function updatebuyerprofile(){
        ...
        if($this->form_validation->run()){
            ...
            $updatedata=array('table'=>'user_Info','where'=>array('id'=>$this->userid),'val'=>array('f_name'=>$f_name,'l_name'=>$l_name,'mobile_no'=>$phone,'state'=>$state,'zip'=>$zip,'address1'=>$address));                
            $updatelog=$this->common->update_data($updatedata);
