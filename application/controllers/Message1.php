...
class Message1 extends MY_Controller {
   ...
   /* public function get_online_user()
    ...
    public function get_online_user()
    {
        ...
        $sql="SELECT DISTINCT chat.from as msgfrom FROM chat WHERE chat.to='".$this->userid."' AND recd='0'";
        ...
        $this->db->select('count(c.id) as noofmsg,b.id as id,b.f_name as first_name,b.l_name as last_name,b.username,b.profile_Pic,b.is_login,us.business_name,b.type_Of_User',FALSE);
        $this->db->join('chat c','b.id=c.from','left');
        $this->db->join('user_store_info us','b.id=us.user_id','left');
        $this->db->where('c.to', $this->userid);
        $this->db->where('c.recd', '0');
        ...
        //$this->db->select('b.id as id,b.is_login',FALSE);
        $this->db->select('count(c.id) as noofmsg,b.id as id,b.f_name as first_name,b.l_name as last_name,b.username,b.profile_Pic,b.is_login,us.business_name,b.type_Of_User',FALSE);
        $this->db->join('chat c',"b.id=c.from and c.recd=0 and c.to=$this->userid",'left');
        $this->db->join('user_store_info us','b.id=us.user_id','left');
        //$this->db->where('c.to', $this->userid);
        //$this->db->where('c.recd', '0');
        $this->db->where('b.id!=', $this->userid);
...
/*    public function get_all_count()
    ...
    public function searchbyusername(){
        ...
        $query="select id from user_Info where (f_name Like '%".$search."%' OR l_name Like '%".$search."%' OR  CONCAT(f_name,' ',l_name) Like '%".$search."%' OR  CONCAT(f_name,'',l_name) Like '%".$search."%' OR username Like '%".$search."%') AND store_info='1' AND id!=".$this->userid;
        ...
        if(!empty($searcheduser)){
            $this->db->select(array("id","f_name","l_name","profile_Pic","username"));
    ...
    public function get_all_count()
    {
        ...
        $this->db->update("user_Info", array("is_login"=>"1","active_time"=>$currenttime));
        //echo $this->db->last_query();exit;
        $where=['to'=>$userid,'recd'=>'0'];
        $this->db->where($where);
        $getcount=$this->db->count_all_results('chat');
        if($getcount > 0)
        {
            echo json_encode(['status'=>TRUE,'message'=>$getcount.' new message arrived.','msgcnt'=>$getcount]);
        }
