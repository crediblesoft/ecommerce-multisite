...
class Bidproduct extends MY_Controller {
    ...
    public function index(){
        $comment1=array('val'=>'p.id,p.prod_name,p.prod_price,p.date,p.admin_status,p.bid_start_date,p.bid_end_date,p.taxable_status,cat.category,u.username,count(btc.id) as bidcount,auc.id as auc_status','table'=>'product as p','where'=>array('bid_status'=>'1'),'minvalue'=>'','group_by'=>'p.id','start'=>'','orderby'=>'p.id','orderas'=>'DESC');
        $multijoin1=array(  
    ...
    public function details($id){
        ...
        $comment1=array('val'=>'p.id,p.prod_name,p.pord_detail,p.prod_img,p.prod_price,p.status,p.admin_status,p.bid_start_date,p.bid_end_date,p.bid_purchase_date,p.weight,p.length,p.width,p.height,p.status,p.taxable_status,p.local_shipping,u.username,u.id as userid,u.type_Of_User,cat.category','table'=>'product as p','where'=>array('p.id'=>$id,'bid_status'=>'1'),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'p.id','orderas'=>'DESC');
        $multijoin1=array(  
            array('table'=>'category as cat','on'=>'p.category=cat.id','join_type'=>''),
            array('table'=>'user_Info as u','on'=>'p.user_id=u.id','join_type'=>'left'),
        );  
    ...
    public function search(){
        ...
        if($searchby=='seller'){
            ...
            $comment1=array('val'=>'p.id,p.prod_name,p.prod_price,p.date,p.admin_status,p.bid_start_date,p.bid_end_date,p.taxable_status,cat.category,u.username,count(btc.id) as bidcount','table'=>'product as p','where'=>$where,'minvalue'=>'','group_by'=>'p.id','start'=>'','orderby'=>'p.user_id','orderas'=>'DESC',"like"=>array());
