...
class Users extends MY_Controller {
    ...
    function __construct()
    {
        parent::__construct();
        $this->_valid_admin();
        $this->_screen_security_admin('Manage Users');
   ...
   //      $comment1=array('val'=>'u.*,slist.state as statename,se.end_date as expiry','table'=>'user_Info as u','where'=>array('u.id'=>$id),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'u.id','orderas'=>'DESC');
   //      $multijoin1=array(  
   //          //array('table'=>'category as cat','on'=>'p.category=cat.id','join_type'=>''),
			// array('table'=>'subscription_expire as se','on'=>'u.id=se.user_id','join_type'=>''),
   ...
   //  }
    public function details($id){
        ...
        $comment1=array('val'=>'u.*,slist.state as statename,se.end_date','table'=>'user_Info as u','where'=>array('u.id'=>$id),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'u.id','orderas'=>'DESC');
        $multijoin1=array(  
            //array('table'=>'category as cat','on'=>'p.category=cat.id','join_type'=>''),
            array('table'=>'statelist as slist','on'=>'u.state=slist.id','join_type'=>'left'),
		 array('table'=>'subscription_expire as se','on'=>'u.id=se.user_id','join_type'=>'left'),
        );    
        $resp['users']=$this->common->multijoin($comment1,$multijoin1);
        //echo "<pre>";
        //print_r($resp); exit;
    ...
    public function update_expiry(){
        ...
        $data=array("table"=>"subscription_expire","val"=>array("end_date"=>$expdate),"where"=>array("user_id"=>$userid));
