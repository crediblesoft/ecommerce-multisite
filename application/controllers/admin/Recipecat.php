...
class Recipecat extends MY_Controller {
    private $userid;
    function __construct()
    {
        parent::__construct();
        $this->_valid_admin();
        $this->_screen_security_admin('Manage Recipe Category');
        $this->userid=$this->session->userdata(ADMIN_SESS.'user_id');
    }
    
    
     public function index(){
        $comment1=array('val'=>'COUNT(r.id) AS `Noofrecipe`,rc.id as Catid,rc.category,rc.status','table'=>'recipe_category as rc','where'=>array(),'minvalue'=>'','group_by'=>'rc.id','start'=>'','orderby'=>'rc.id','orderas'=>'DESC');
        ...
        $config = array();
        $config["base_url"] = BASE_URL. "admin/recipecat";
        ...
        $this->load->view('admin/recipes/admin_vw_recipecat',$log);
    ...
    public function add(){
        ...
        if($this->form_validation->run()){
            ...
            if($log){
                ...
                redirect("admin/recipecat/add/","refresh");
            }
        }else{
        $this->load->view('admin/include/admin_header');
        $this->load->view('admin/include/admin_left');
    ...
    public function updatecategory(){
        ...
        if($this->form_validation->run()){
            ...
            if($log){
                ...
                redirect("admin/recipecat","refresh");
            }
        }else{
    ...
    public function details($rid){
        $comment1=array('table'=>'recipe as rs','val'=>'u.type_Of_User,u.store_info, u.id as userid,u.username,u.f_name,u.l_name,u.profile_Pic,rs.recipe_type, rs.recipe_title, rs.recipe_detail, rs.recipe_addDate, rs.recipe_updateDate,rs.image_path,rs.recipe_stetus,rs.admin_status,rs.id as recipeid,rs.video_link,cat.category','where'=>array("rs.id"=>$rid),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'rs.id','orderas'=>'DESC');
        $multijoin1=array(
            array('table'=>'user_Info as u','on'=>'u.id=rs.user_id','join_type'=>'left'),
            array('table'=>'recipe_category as cat','on'=>'cat.id=rs.recipe_type','join_type'=>'')
        );
    ...
    public function search($catid=null){
        ...
        if($searchby=='category'){
            ...
            $comment1=array('table'=>'recipe as rs','val'=>'u.type_Of_User,u.store_info, u.id as userid,u.username,u.f_name,u.l_name,u.profile_Pic,rs.recipe_type, rs.recipe_title, rs.recipe_detail, rs.recipe_addDate, rs.recipe_updateDate,rs.image_path,rs.recipe_stetus,rs.admin_status,rs.id as recipeid,cat.category','where'=>$where,'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'rs.id','orderas'=>'DESC',"like"=>array());
