...
class Products extends MY_Controller {
    ...
    public function index($catid,$sub_catid=0){
        //$this->send_mail_buyer();exit;
        //$this->send_mail_seller(); exit;
        //echo $catid." ".$sub_catid;exit;
        ...
        $comment2=array('val'=>'u.f_name,u.l_name,u.username,u.id as userid,u.profile_Pic,u.state','table'=>'product as p','where'=>array("p.category"=>$catid,"p.category_sub"=>$sub_catid,"p.status"=>"1"),'minvalue'=>'','group_by'=>'u.id','start'=>'','orderby'=>'u.paid','orderas'=>'DESC');
...
//            array('table'=>'user_Info as u','on'=>'ubt.user_id=u.id','join_type'=>''),
	...
	$resp['category']= $this->getcategory($catid);
        ...
        if($sub_catid!=0){
          $query3=array("table"=>"sub_category","val"=>array("sub_category","sub_description"),"where"=>array("sub_catid "=>$sub_catid));
          $resp['sub_category']=$this->common->getsinglerow($query3);
          $resp['sub_categoryid']= $sub_catid;
         }
       ...
       // print_r($resp); exit;
        ...
        $data=array("min"=>array("field"=>'prod_price',"as"=>'min'),"max"=>array("field"=>'prod_price',"as"=>'max'),"table"=>"product","where"=>array("category"=>$catid,"category_sub"=>$sub_catid,"status"=>"1","no_of_Prod > "=> '0','admin_status'=>'1'));
        $resp['price']=$this->common->getminmax($data);          

        $table=$this->common->multijoin($comment1,$multijoin1);
        $config = array();
        if($sub_catid!=0){
           $config["base_url"] = BASE_URL. "products/$catid-$sub_catid";
        }else{
...
//            }
    ...
    public function details($prodId){        
       ...
       if(!$log22['res']){
        ...
        $current_date=date("Y-m-d");
        $comment1=array('val'=>'p.id as prod_id,p.category,p.prod_name,p.prod_price,p.no_of_Prod,p.status,p.prod_img,p.bid_status,p.pord_detail,p.bid_end_date,usi.certification,usi.size,usi.business_name,usi.address,usi.zip,usi.city,u.id as userid,u.username,s.state,u.is_login as selleronline,GROUP_CONCAT(bt.category SEPARATOR ",") as business_type','table'=>'product as p','where'=>array("p.id"=>$prodId,"p.status"=>"1",'p.admin_status'=>'1','u.status'=>'1'),'minvalue'=>'','group_by'=>'p.id','start'=>'','orderby'=>'p.id','orderas'=>'DESC');
        $multijoin1=array(  
...
//print_r($allcart);exit;
    ...
    public function filter(){
        ...
        $sub_categoryid=$this->input->post("sub_categoryid");
        ...
        if($by=='category'){$where=array("p.category"=>$categoryid,"p.category_sub"=>$sub_categoryid); }else{$where=array("p.user_id"=>$userid);}
        ...
        if($by=='category'){ 
           if($sub_catid==0){
              ...
              $config["base_url"] = BASE_URL. "products/$categoryid"; 
            }
        }else{
           $config["base_url"] = BASE_URL. "sellerproduct/$userid";
        }
        $config["total_rows"] = ($table['res'])?count($table['rows']):0;
        $config["per_page"] = 9;
...
//        $config['prev_tag_close'] = '</li>';
	...
	public function searchbydistance($catid){
        ...
        if(($zip!='' || $city!='') && $distance!=''){
            ...
            $comment1=array('val'=>'count(p.id) as ct,p.id as prod_id,p.prod_name,p.prod_price,p.no_of_Prod,p.status,p.prod_img,p.bid_status,p.pord_detail,usi.certification,usi.size,usi.business_name,usi.city,u.username','table'=>'product as p','where'=>array("p.category"=>$catid,"p.status"=>"1","p.no_of_Prod >"=>'0','p.admin_status'=>'1','u.status'=>'1'),"or_where"=>array("p.bid_end_date >="=>$current_date,"p.bid_end_date ="=>"0000-00-00"),'minvalue'=>'','group_by'=>'p.id','start'=>'','orderby'=>'u.paid','orderas'=>'DESC','in'=>'','in_value'=>'');
            $multijoin1=array(  
                array('table'=>'category as cat','on'=>'p.category=cat.id','join_type'=>''),
                array('table'=>'user_Info as u','on'=>'p.user_id=u.id','join_type'=>''),
                array('table'=>'user_store_info as usi','on'=>'p.user_id=usi.user_id','join_type'=>''),
