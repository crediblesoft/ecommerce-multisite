...
class Mail extends MY_Controller {
    ...
    public function detail($mailid,$mailtoid=0){
        ...
        $comment1=array('val'=>'m.id as mail_id,m.subject,m.message,m.timestamp,m.attach,u.f_name,u.l_name,u.username,m.status','table'=>'mail as m','where'=>array("m.id"=>$mailid),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'m.id','orderas'=>'DESC');
        $multijoin1=array(  
        ...
        $comment1=array('val'=>'u.f_name,u.l_name,u.username,mt.status','table'=>'mail_to as mt','where'=>array("mt.mail_from"=>$mailid),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'mt.id','orderas'=>'DESC');
        $multijoin1=array(  
            //array('table'=>'mail_to as mt','on'=>'m.id=mt.mail_from','join_type'=>''),
            array('table'=>'user_Info as u','on'=>'mt.mail_to=u.id','join_type'=>''),
        );
    ...
    public function attach(){
        ...
        for($i=0;$i<count($_FILES);$i++){
            ...
            $date = time();
            $filename1=$_FILES[$i]['name'];

            $filename=$date.'_'.$filename1;
  //echo $filename; exit;
   ...
   //echo $destination;         
            ...
            if($upload){
                echo '';
                 ...
                 echo json_encode(array('status'=>true,'result'=>$filename));
            }
