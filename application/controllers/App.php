...
class App extends MY_Controller
{
            ...
            if($result['res'])
            {
				...
				else
				{
					if($result['rows'][0]->status=='1'){
						...
						$query1="update user_Info SET `is_login`='1',`login_time`='".time()."' where `email_id`='".$email."' or `username`='".$email."'";
    ...
    {   
           ...
           $query1="update user_Info SET `is_login`='0' where `id`='".$uid."'";
	//echo $query1;exit;
	   $result1=$this->db->query($query1);
	if($result1){
		echo "yes";
    ...
    function profile()
    {
       ...
       $result=$this->common->dbQuery($query);
		...
		$qr2="select u.business_id,c.category from user_business_type as u left join category as c on c.id=u.business_id where user_id='".$uid."' ";
		$buss=$this->common->dbQuery($qr2);
		
		$qr3="select * from `theme_price` where price_for='seller' and status='1' ";
		$plan=$this->common->dbQuery($qr3);
		$comment2=array('val'=>'*,tp.type','table'=>'subscription_expire as sue','where'=>array('sue.user_id'=>$uid,'sue.status'=>'1'),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'sue.int','orderas'=>'DESC');
            $multijoin2=array(
                array('table'=>'theme_price as tp','on'=>'sue.package_id=tp.id','join_type'=>'left'),
            );
            $paiduser=$this->common->multijoin($comment2,$multijoin2); 
		//echo $this->db->last_query();
		//print_r($paiduser); exit;
		if($seller_result['res'])
		{
			   
				echo json_encode(array('status'=>true,'result'=>$seller_result['rows'],'bussiness'=>$buss['rows'],'plan'=>$plan['rows'],'paiduser'=>$paiduser['rows'],'social'=>$social['rows']));
			}
			else{
	...
	function update_store()
	{
		header('Access-Control-Allow-Origin: *');
            ...
            /*if(!$sub_merchant_account['status']){ 
                 ...
                 $data=array('table'=>'user_store_info','val'=>array('user_id'=>$uid,'business_name'=>$bname,'contact_person_name'=>$cp_name,'phone'=>$store_phone,'address'=> $store_address,'zip'=>$zip,"income"=>$income,'certification'=>$cert));                
                 $log=$this->common->add_data($data); 
...
$comment1=array('table'=>'recipe as rs','val'=>'u.status as user_status,u.type_Of_User,u.store_info, u.id,u.username,u.f_name,u.l_name,u.profile_Pic,rs.recipe_type, rs.recipe_title, rs.recipe_detail, rs.recipe_addDate, rs.recipe_updateDate,rs.image_path,rs.id as recipeid,cat.category','where'=>array("rs.recipe_stetus"=>'1',"rs.admin_status"=>"1"),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'rs.id','orderas'=>'DESC');
	...
	function recipe_view()
	{
		...
		//$uid=$this->input->post('id');
                $comment1=array('table'=>'user_Info as u','val'=>'u.status as user_status,u.type_Of_User,u.store_info,u.id,u.username,u.is_login as useronline,GROUP_CONCAT(bt.category SEPARATOR ",") as business_type,usi.business_name,u.f_name,u.l_name,u.profile_Pic,rs.recipe_type,rs.recipe_title,rs.video_link,rs.recipe_detail, rs.recipe_addDate,rs.recipe_updateDate,rs.image_path,rs.id as recipeid,rs.recipe_stetus,rs.user_id,cat.category','where'=>array("rs.id"=>$recp_id),'minvalue'=>'','group_by'=>'rs.id','start'=>'','orderby'=>'','orderas'=>'');
        ...
        $multijoin1=array(
            array('table'=>'recipe as rs','on'=>'u.id=rs.user_id','join_type'=>'right'), 
            array('table'=>'recipe_category as cat','on'=>'cat.id=rs.recipe_type','join_type'=>'right'),
...
function search_recipe(){
		...
		$in=array();$in_value=array();
		$where=array("rs.recipe_stetus"=>'1',"rs.admin_status"=>"1");
		$like=array("likeon"=>'rs.recipe_title',"likeval"=>$title);
	...
	function product_view()
	{
		...
		$query="SELECT p.*,c.category as cat_name,usi.business_name,ui.address1,s.state,ui.zip,ui.username,ui.is_login from product p join user_Info ui left join user_store_info usi on p.user_id=ui.id LEFT JOIN category c ON c.id = p.category LEFT JOIN statelist s ON s.id = ui.state where p.id='".$prod_id."' and usi.user_id=p.user_id";
                        $result=$this->common->dbQuery($query);
		//echo $this->db->last_query(); exit;	         
            if($result['res'])
            {
	...
	function seller_orders(){
		...
		$userid=$this->input->post('uid');
		 ...
		 $comment1=array('val'=>'t.trans_id,(ts.total)+((ts.total*ts.tax)/100)+(ts.shippingcharge) as price,t.price as trans_price,t.date,u.f_name,u.l_name,u.id as buyerId','table'=>'transaction as t','where'=>array('p.user_id'=>$userid,'t.payment_for'=>'product'),'minvalue'=>'','group_by'=>'o.trans_id','start'=>'','orderby'=>'t.id','orderas'=>'DESC');
        $multijoin1=array(  
            ...
            array('table'=>'transaction_sellers as ts','on'=>$userid.'=ts.seller_id AND ts.trans_id=t.trans_id','join_type'=>'left'),
        );
	...
	//echo $this->db->last_query();exit;
	 ...
	 function seller_orders_search(){
  header('Access-Control-Allow-Origin: *');
  $userid=$this->input->post('uid');
  $transId=$this->input->post('transId');
        $userid=$this->input->post('uid');
	
$comment1=array('val'=>'t.trans_id,o.id,o.status,o.price,o.quantity,o.date,p.id as prodId,p.prod_name,p.prod_img as prod_img,u.f_name,u.l_name,u.id as buyerIdg','table'=>'transaction as t','where'=>array('p.user_id'=>$userid),'minvalue'=>'','group_by'=>'o.trans_id','start'=>'','orderby'=>'t.id','orderas'=>'DESC','like'=>array('likeon'=>'t.trans_id','likeval'=>$transId));
        $multijoin1=array(  
            array('table'=>'order_detail_form as o','on'=>'o.trans_id=t.trans_id','join_type'=>'left'),
            array('table'=>'user_Info as u','on'=>'o.buyerId=u.id','join_type'=>'left'),
            array('table'=>'product as p','on'=>'o.product_id=p.id','join_type'=>'left'),
...
//print_r($result); exit;
	...
	function buyer_bidproducts(){
		...
		$newresult=array();
		foreach($result['rows'] as $productlist){
           $sql="select * from product_auction where prod_id=$productlist->prod_id and status='1'";
            ...
            //echo "<pre>"; print_r($result); 
                   ...
                   $auctionid=$resp['winner']['rows'][0]->id;
                    ...
                    $newlist=(array)$productlist;
                    $newlist['winner']=$buyer_id;
                    $newresult[] = (object) $newlist;
                }

		if(!empty($newresult))
            {
				echo json_encode(array('status'=>true,'result'=>$newresult));
            }
            else{
                echo json_encode(array('status'=>FALSE));
            }
	...
	{	
        ...
        $sql="select bid.price,u.id as userid,u.type_Of_User,u.username,u.mobile_no,u.email_id,u.f_name,u.l_name,u.address1 from bid_tbl_cart as bid left join user_Info as u on bid.user_id=u.id where bid.auction='".$auctionid."' AND bid.price=(SELECT MAX(price) FROM bid_tbl_cart where auction='".$auctionid."')";
        $query=$this->db->query($sql);
		 $result=$query->result();
		...
		$this->db->select('t.trans_id,ts.*');
		 ...
		 $trans=$this->db->get('transaction t');
		//$trans=$this->common->dbQuery($qr1);
		if($trans -> num_rows() > 0){$trans1=array('res'=>true,'rows'=>$trans->row());}
            else{$trans1=array('res'=>false);}
		
		//echo $this->db->last_query();exit;
       
        if($result){
            echo json_encode(array('status'=>true,'result'=>$result,'trans'=>$trans1));
        }else{
...
function viewcart(){
	...
	function tax_calcualtion_for_bid($auctionid,$sellerid,$uid,$toship){
        ...
        $this->db->select('usi.merchant_id,u.f_name,u.l_name,p.weight,p.local_shipping,p.taxable_status,usi.zip,s.code,(max(btc.price)) as prod_total,p.user_id,if(p.taxable_status="1",if(u.state='.$buyerstate->state.',IFNULL(dt.total,"0"),"0"),"0") as tax,if(u.state='.$buyerstate->state.',dt.details,"Tax not charged") as desc,(((max(btc.price))*(if(p.taxable_status="1",if(u.state ='.$buyerstate->state.',IFNULL(dt.total,"0"),"0"),"0")))/100) as sub_total_tax,((max(btc.price))+((((max(btc.price))*(if(p.taxable_status="1",if(u.state ='.$buyerstate->state.',IFNULL(dt.total,"0"),"0"),"0")))/100))) as sub_total');
...
//                'size_units' => 'in', // in (default), feet, cm
	...
	{ 
			...
			$paymentdata['package_id']=$this->input->post('package_id');
		...
		{  
						...
						if($payfor=='product'){
							...
							$paymentdata['promocode']=$this->input->post('promocode');
    ...
    { 
		...
		$finaldata1=$this->tax_calculation($data['uid'],$data['zipCode']);
		//print_r($finaldata1);exit;
	 ...
	 $this->send_mail_buyer_app($data['tokan'],$data['uid']);
           $this->send_mail_seller_app($data['tokan']); 
	...
	function card_payment_insert_user($data)
    {
           ...
           $data1=array('table'=>'user_Info','val'=>array('paid'=>'1','promocode'=>$data['promocode']),'where'=>array('id'=>$data['uid']));
           ...
           $package=array('val'=>'*','table'=>'theme_price','where'=>array('price_for'=>'seller','id'=>$data['package_id']));
		   ...
		   $sub_data=array('table'=>'subscription_expire','val'=>array('user_id'=>$data['uid'],'start_date'=>$date,'end_date'=>$expire,'package_id'=>$data['package_id']));
	...
	/*Mail send to buyer start*/
     function send_mail_buyer_app($trans_id,$to){
        ...
        $table=$this->common->multijoin($comment1,$multijoin1);
          ...
          $comment1=array('val'=>'udf.price,udf.product_id,(udf.price*udf.quantity) as totalp,if(p.taxable_status="1",((udf.price*udf.quantity)*ts.tax)/100,"0") as taxi','table'=>'order_detail_form as udf','where'=>array("udf.trans_id"=>$trans_id),'group_by'=>'','minvalue'=>'','start'=>'','orderby'=>'udf.id','orderas'=>'DESC');
         $multijoin1=array(
             array('table'=>'product as p','on'=>'udf.product_id=p.id','join_type'=>'left'),
             array('table'=>'transaction_sellers as ts','on'=>'ts.trans_id=udf.trans_id and p.user_id=ts.seller_id','join_type'=>'left'),
         );
        
        $table_1=$this->common->multijoin($comment1,$multijoin1);
        
        
        $comment2="SELECT sum(ts.shippingcharge) as totalshiping FROM transaction_sellers as ts where ts.trans_id='$trans_id'";
        $table_2=$this->common->dbQuery($comment2);
       ...
       // $ftax+=($nttl*$tax)/100;
        ...
        $message='<h2>Invoice</h2></br>
            ...
            <table class="table table-bordered" style="border:1px solid black;">
                            ...
                            <tr style="width:20%;"><td colspan="3">Shipping Charges:-</td><td>$'.$table_2['rows'][0]->totalshiping.'</td></tr>        
...
//        echo "<pre>";
    ...
    function send_mail_seller_app($trans_id){
       $comment1=array('val'=>'ts.seller_id,ts.trans_id,ts.commission,ts.shippingcharge,u.email_id','table'=>'transaction_sellers as ts','where'=>array("ts.trans_id"=>$trans_id),'group_by'=>'','minvalue'=>'','start'=>'','orderby'=>'ts.id','orderas'=>'DESC');
       $multijoin1=array(
            array('table'=>'transaction as t','on'=>'ts.trans_id=t.trans_id','join_type'=>'left'),
           array('table'=>'user_Info as u','on'=>'ts.seller_id=u.id','join_type'=>'left'),
            
        );
      $sellers=$this->common->multijoin($comment1,$multijoin1);
      //echo "<pre>";
      if($sellers['res']){
      foreach ($sellers['rows'] as $seller){
        ...
        $tto=$seller->email_id;
         // print_r($seller);
         $comment1=array('val'=>'p.prod_name,udf.price,udf.product_id,udf.quantity,(udf.price*udf.quantity) as totalp,if(p.taxable_status="1",((udf.price*udf.quantity)*ts.tax)/100,"0") as taxi','table'=>'order_detail_form as udf','where'=>array("udf.trans_id"=>$trans_id,"ts.seller_id="=>$seller->seller_id),'group_by'=>'','minvalue'=>'','start'=>'','orderby'=>'udf.id','orderas'=>'DESC');
         $multijoin1=array(
             array('table'=>'product as p','on'=>'udf.product_id=p.id','join_type'=>'left'),
             array('table'=>'transaction_sellers as ts','on'=>'ts.trans_id=udf.trans_id and p.user_id=ts.seller_id','join_type'=>'left'),
         );
         $productdetails=$this->common->multijoin($comment1,$multijoin1);
         //echo "<pre>";
         //print_r($productdetails);
          $list='';$sellertax=0;$ptotal=0;$gtotal=0;$pay_to_seller=0;
         foreach ($productdetails['rows'] as $productdetail){
         $list.='<tr><td style="width:40%;">'.$productdetail->prod_name.'</td><td style="width:20%;">$'.$productdetail->price.'</td><td  style="width:20%;">'.$productdetail->quantity.'</td><td>$'.$productdetail->totalp.'</td></tr>';   
         $sellertax+=$productdetail->taxi;
         $ptotal+=$productdetail->totalp;
         $gtotal=($ptotal+$seller->shippingcharge)+$sellertax;
         $pay_to_seller=$gtotal-$seller->commission;
        ...
        $message='<h2>Invoice</h2></br>
            <h4>Transaction-Id:-'.$seller->trans_id.'</h4></br>
            <table class="table table-bordered"  style="border:1px solid black;">
                            <thead>
                              <tr>
                                <th style="width:40%; text-align: left;">Product</th>
                            ...
                            <tbody>'.$list.'
                            <tr style="width:20%; border-top:1px solid black;"><td colspan="3">Tax</td><td>$'.$sellertax.'</td></tr>
                            <tr style="width:20%; border-top:1px solid black;"><td colspan="3">Shipping Charges:-</td><td>$'.$seller->shippingcharge.'</td></tr>       
                            <tr style="width:20%; border-top:1px solid black;"><td colspan="3">Grand Total:-</td><td>$'.$gtotal.'</td></tr>   
                            <tr style="width:20%; border-top:1px solid black;"><td colspan="3">Commission(-)</td><td>$'.$seller->commission.'</td></tr>
	...
	function bid_details()
	{
		...
		$comment2=array('val'=>'bid.price,bid.add_date,u.f_name,u.l_name,bid.auction','table'=>'bid_tbl_cart as bid','where'=>array("bid.auction"=>$auctionid),'minvalue'=>'','group_by'=>'','start'=>'','orderby'=>'bid.price','orderas'=>'DESC');
		$multijoin2=array(  
			array('table'=>'user_Info as u','on'=>'bid.user_id=u.id','join_type'=>''),
		);
		$result=$this->common->multijoin($comment2,$multijoin2);
		if($result['res'])
            {
			  echo json_encode(array('status'=>true,'result'=>$result['rows']));
            }
	...
	function update_product()
	{
		...
		$data=array('table'=>'product','where'=>array('id'=>$prodId),'val'=>array('category'=>$category,'prod_name'=>$name,'prod_price'=>$price,'no_of_Prod'=>$quantity,'pord_detail'=>$details,'bid_status'=>$selltype,"bid_start_date"=>$bid_start_date,"bid_end_date"=>$bid_end_date,"bid_purchase_date"=>$bid_purchase_date,'weight'=>$weight,'weight_unit'=>$weight_unit,'length'=>$length,'width'=>$width,'height'=>$height,'die_unit'=>$die_unit,'status'=>$status,'prod_img'=>$prodImage,'taxable_status'=>$taxable,'local_shipping'=>$shipping));                
		$log=$this->common->update_data($data);
	...
	function userlimitation()
	{
	 ...
	 function buyer_orders(){
		header('Access-Control-Allow-Origin: *');
		$userid=$this->input->post('uid');
      /* $comment1=array('val'=>"t.trans_id,t.price,t.date,u.f_name,u.l_name,u.id as buyerId","table"=>"transaction as t",
"where"=>array("o.buyerId"=>$userid,"t.payment_for"=>"product"),
"or"=>array("t.payment_for="=>"bidproduct"),
"minvalue"=>"","group_by"=>"o.trans_id","start"=>"","orderby"=>"t.id","orderas"=>"DESC");*/

		$this->db->select('t.trans_id,t.price,t.date,u.f_name,u.l_name,u.id as buyerId,o.buyerId as tt');
		$this->db->join('order_detail_form as o','o.trans_id=t.trans_id','left');
		$this->db->join('product as p','o.product_id=p.id','left');
		$this->db->join('user_Info as u','p.user_id=u.id','left');
		$this->db->where("o.buyerId =",$userid);
		$this->db->group_start();
		$this->db->or_where("t.payment_for =","product");
		$this->db->or_where("t.payment_for =","bidproduct");
		$this->db->group_end();
		$this->db->group_by("o.trans_id");
		$this->db->order_by("t.id");
		$query=$this->db->get('transaction as t');
...
//echo $this->db->last_query();exit;
		...
		if($query->num_rows() > 0) 
            	{
			  echo json_encode(array('status'=>true,'result'=>$query->result()));
            	}else{
			echo json_encode(array('status'=>false,));
		}
    }
...
//       }
    ...
    function search_sellerrequirement(){
	$userid=$this->input->post('uid');
	$likeval=$this->input->post('searchtext');
...
function uploadcampaignpic(){
		...
		//copy($_FILES['file']['tmp_name'],'assets/image/product/thumb/'.$image);
		$img=getimagesize('assets/image/campaign/'.$image);
		//echo $img; exit;
		if(!empty($img))
		{
			$imgtype= $img['mime'];
...
public function get_online_user()
    ...
    { 
            ...
            $cruserid=$this->input->post("crid");
            $data=['recd'=>'1'];
            $where=['recd'=>'0','to'=>$cruserid,'from'=>$userid];
            $this->db->where($where);
            $this->db->update('chat',$data);
            $sql="SELECT a.*,c.f_name as first_name,c.l_name as last_name,c.profile_Pic,c.is_login FROM chat as a LEFT JOIN user_Info as c ON a.from=c.id WHERE (a.from='".$userid."' OR a.to='".$userid."') AND (a.from='".$cruserid."' OR a.to='".$cruserid."') ORDER BY a.sent ASC";
            $query=$this->db->query($sql);
            if($query -> num_rows()>0){
            $result=$query->result();
	 ...
	 public function get_all_count()
    {
        ...
        //echo $this->db->last_query();exit;
        $where=['msgfor'=>$userid,'status'=>'0','msgfrom'=>$cruserid];
        $this->db->where($where);
        $getcount=$this->db->count_all_results('conservation');
		// echo $getcount;exit;
        if($getcount > 0)
    ...
    public function get_all_count_show()
    {
        $currenttime= date('Y-m-d H:i:s');
        $userid=$this->input->post("uid");
		$cruserid=$this->input->post("crid");
        ...
        $this->db->update("user_Info", array("is_login"=>"1","active_time"=>$currenttime));
        //echo $this->db->last_query();exit;
        $where=['to'=>$userid,'status'=>'0','from'=>$cruserid];
        $this->db->where($where);
        $getcount=$this->db->update('chat', array("status"=>"1",));
		// echo $getcount;exit;
        if($getcount)
        {
    ...
    { 
            ...
            $query=$this->db->query($sql);
	    $sql1="SELECT c.f_name as first_name,c.l_name as last_name,c.profile_Pic,c.is_login FROM user_Info as c where c.id='".$cruserid."'";
		 $query1=$this->db->query($sql1);
		if($query1 -> num_rows()>0){
			$result1=$query1->result();
		}else{$result1="";}
            if($query -> num_rows()>0){
            $result=$query->result();
                echo json_encode(['status'=>TRUE,'message'=>$result,'info'=>$result1]);
            }else{
                echo json_encode(['status'=>FALSE,'message'=>'','info'=>$result1]);
            }
    }
public function send()
    {
        $userid=$this->input->post("uid");
            $cruserid=$this->input->post("crid");
             $user_paid=$this->input->post("user_paid");
             $valid=$this->_usermessagelimitation("message",$user_paid,"chat",array("from"=>$userid));
           
            $data=['from'=>$userid,'to'=>$cruserid,'message'=>$this->input->post('message'),'recd'=>'0','sent'=>date('Y-m-d H:i:s')];
...
public function _usermessagelimitation($title=null,$userpaidstatus=null,$table=null,$where=array()){
            ...
            if($log['res']){
                ...
                if($log['rows']->title > $newlog){
                    ...
                    $count=$log['rows']->title;
                    //$message="you have only $count $title limitation for more $title you need to go(profile->membership..) for purchase Theame";
                    $message="As a Free User, you are limited to send $count $title. To send more and access additional functionality, please go to profile, then membership to purchase a Premium package.";
    ...
    public function user_paid_status(){
		...
		if($log['res'])
        {
             echo json_encode(array('status'=>true,'data'=>$log['rows']));
        }
 ...
 public function searchbyusername(){
	...
	$uid=$this->input->post("uid");
        $query="select id from user_Info where (f_name Like '%".$search."%' OR l_name Like '%".$search."%' OR  CONCAT(f_name,' ',l_name) Like '%".$search."%' OR  CONCAT(f_name,'',l_name) Like '%".$search."%' OR username Like '%".$search."%') AND store_info='1' AND id !='".$uid."'";
	...
	//echo $this->input->post("page");exit;
     ...
     public function search_send($users=array(),$user){
       ...
       if($this->input->post('currentpage')){
		...
		$page = $pg;
		//echo $this->db->last_query();
        	$log['sendlist']=$this->common->multijoin_with_in($comment1,$multijoin1,$config["per_page"], $page);
	        $log["links"] = $this->pagination->create_links();
		$log['datashowcount'] = array('total' => $config["total_rows"], 'per_page' => $config["per_page"]);
		//echo $this->db->last_query();
		//print_r($table);exit;
		if($table['res'])
		    {
		        echo json_encode(array('status'=>true,'mail_detail'=> $log['sendlist'],'links'=>$log["links"],'datashowcount'=>$log['datashowcount']));
		    }
		    else{
    ...
    public function search_trash($users=array(),$user){
        ...
        $log['trash_mail']=$this->common->multijoin_with_in($comment2,$multijoin2);
         ...
         if($log['trash_mail_to']['res']> 0 or $log['trash_mail']['res']> 0)
            {
                echo json_encode(array('status'=>true,'mail_to'=> $log['trash_mail_to'],'mail_send'=> $log['trash_mail'],'links'=>$log["links"],'datashowcount'=>$log['datashowcount']));
            }
            else{
...
public function _media_userlimitation($title=null,$userpaidstatus=null,$table=null,$where=array(),$redirectpage){
            ...
            if($log['res']){          
                ...
                $newdata1=array("table"=>"user_videos","where"=>array("user_id"=>$userid));
                ...
                if($log['rows']->title > ($newlog+$newlog1)){
                    ...
                    $count=$log['rows']->title;
                    $message= "Warning,As a Free User, you are limited to posting ".$count." media.  To post more and access additional functionality, please go to profile, then membership to purchase a Premium package.";
...
public function files(){
	...
	$uid=$this->input->post('uid');    
	$user_paid=$this->input->post('user_paid');
	$aa=$this->_media_userlimitation("media",$user_paid,"userProd_image",array("user_id"=>$uid,"type_ofimage"=>"gallery"),"gallery");
	if($aa['status']==0){
		 	echo json_encode(array('status'=>FALSE,'message'=> $aa['message']));
			exit;
			}
...
public function addvideos(){
        ...
        $user_paid=$this->input->post('user_paid');
         $aa=$this->_media_userlimitation("media",$user_paid,"userProd_image",array("user_id"=>$userid,"type_ofimage"=>"gallery"),"gallery");
            if($aa['status']==0){
		 	echo json_encode(array('status'=>FALSE,'message'=> $aa['message']));
			exit;
			}
...
public function video_update(){
 ...
 public function seller_search_expense()
    {
	//print_r($_POST);exit;
	// header('Access-Control-Allow-Origin: *');
	 $uid=$this->input->post('uid');  
          ...
          if($result)
            {
			  echo json_encode(array('status'=>true,'result'=>$result));
                ...
                echo json_encode(array('status'=>FALSE,'message'=>'Sorry no record found'));
            }
...
// Niraj
 ...
 function buyer_orders_search(){
  ...
  if($result['res'])
	...
	function autosearch_user(){
		...
		$autosearch=$this->input->post('autosearch');
		$like=array(
			array("likeon"=>"username","likeval"=>$autosearch),
 ...
 public function seller_order_details(){
        ...
        $where3=array('tax.trans_id'=>$transid);if($this->input->post('user_type')=="1"){$where3['tax.seller_id']=$userid;}
        ...
        //echo $this->db->last_query();exit;
        $where=array('o.trans_id'=>$transid);if($this->input->post('user_type')=="1"){$where['p.user_id']=$userid;}
        ...
        $resp['order']=$this->common->multijoin($comment1,$multijoin1);
       
// echo "<pre>";
       // print_r($resp['order']); exit;
        if($resp['order']['res'])
            {
			  
			  echo json_encode(array('status'=>true,'result'=>$resp));
            }
            else{
                echo json_encode(array('status'=>FALSE));
            }
