...
class MY_Controller extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->commondata['businesstype']=$businesstype;
    ...
    public function getbusinesstype($busid=NULL){
        if($busid==NULL){
            $data=array("table"=>"business_types","val"=>array(),"where"=>array("status"=>"active"));
            return $log=$this->common->getdata($data);
        }else{
            $data=array("table"=>"business_types","val"=>array("business_type_name"),"where"=>array("id"=>$busid));
            return $log=$this->common->getdata($data);
        }
    }
    ...
    public function _checkvalid($page){
         if(!$this->session->has_userdata('user_id')){
            ...
            $this->session->set_flashdata("warning","Please Login to proceed.");
            redirect("auth/login");
    }
    }
    
    public function invalid_user(){
        $this->session->set_flashdata("warning","You are not authorized for this feature.");
	...
	public function is_buyer($msg='You have to login as a buyer to access this feature.'){
    ...
    public function is_seller($msg='You have to login as a seller to access this feature.'){
    ...
    function _screen_security_admin($security_name){ 
        $admin_type=$this->session->userdata(ADMIN_SESS.'admin_type');
        $security=$this->session->userdata(ADMIN_SESS.$security_name);
        $key_name=$security_name;

        if($admin_type!='1') {
          if($security!=$key_name) {
         $this->session->set_flashdata("warning","Access Denied: You do not have Security Rights to this screen");
