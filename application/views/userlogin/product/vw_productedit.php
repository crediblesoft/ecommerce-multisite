                ...
                <div class="">
                    <form class="form-horizontal" role="form" enctype = 'multipart/form-data' method="post" action="<?=BASE_URL?>product/update">
                        <input type='hidden' value="<?=$product->id?>" name="productid">
                            ...
                            <div class="form-group required">
                                <div class="col-sm-3 col-sm-offset-1"><label class="control-label pull-left" for="email">Sub-Category</label></div>
                              <div class="col-sm-8">
                                  <select class="form-control" id="sub_category" name="sub_category">
                        <?php 
                        $query="SELECT * FROM sub_category WHERE id=".$product->category;
                        $result=$this->db->query($query);$rows=$result->result();
                        if(count($rows)>0){
                          foreach($rows as $subcat){
                        ?>
                         <option value="<?=$subcat->sub_catid?>" <?php if($product->category_sub==$subcat->sub_catid){echo "selected";} ?> ><?=$subcat->sub_category?></option>
                        <?php } } ?>

                                </select>
                                  <?php if(form_error('sub_category')!='') echo form_error('sub_category','<div class="text-danger err">','</div>'); ?>
                              </div>
...
<script>
      ...
      $(document).on("change","#category",function(){
            ...
            $.post("<?=BASE_URL?>product/getsubcategory",{catidval:catidval},function(data,status){
                var obj = jQuery.parseJSON(data);
            //alert("Data from getsubcategory "+obj);
            $("#sub_category").find("option").remove();
            if(obj.subcatlist.res){
                //console.log(obj.rows);
                $.each(obj.subcatlist.rows,function(i,val){
                     //alert("In each loop");
                     $('#sub_category').append($('<option>', { value: val.sub_catid}).text(val.sub_category));
                 });
                }
            });
      });
