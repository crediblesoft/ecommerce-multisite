<div class="col-sm-9">
    <div class="">
            ...
            <div class="contant-body1">
                <div class="col-sm-12">
                    <div class="">
                    <div class="table-responsive padding_left_none padding_right_none">
...
$(".show_spinner").click(function(){
        ...
        $(".winner").click(function(){
            var id=$(this).prop('id');
            var tabledata="";
            $.post("<?=BASE_URL?>product/getwinnernew",{id:id},function(data,status){      
                
                //console.log(data);
                var obj = jQuery.parseJSON(data);
               
                if(obj.res){
                    var user=obj.data.data[0];
                    var trans=obj.data.trans;
                    var auc_id=user.pro_aucid;
                    ...
                    tabledata+="<tr>";
                    if(user.buyer_id=='<?php echo $this->session->userdata("user_id")?>' && '<?php echo $currentdate=date('Y-m-d')?>'<=user.bid_purchase_date && trans.res==false){
                      //alert("in");
                      //alert(user.trans_id);
                    tabledata+="<tr>";
                    //alert(auc_id);
                    tabledata+="<td></td>"
                    tabledata+='<td><a href="<?php echo BASE_URL?>bid/pricedetails/'+auc_id+'/'+seller_id+'" id="write_review" class="btn btn-success btn-sm show_spinner"> Pay Now</a></td>';
                    tabledata+="<tr>";  
                    }    
                }else{
                    tabledata+="<tr>";
