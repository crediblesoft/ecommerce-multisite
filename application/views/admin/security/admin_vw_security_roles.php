        ...
        <div class="col-md-12">
          <div class="box">
            <div class="box-header with-border">
              ...
              <a href="<?=BASE_URL?>admin/security/add_role" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-plus-sign"></span> Add Security Role</a>
              ...
              <table class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <th>Security Roles Names</th>
                        <th>Security Roles Description</th>
                        <th>Status</th>
                        <th colspan="2" style="width: 12px">Action</th>
                    </tr>
                    
                    <?php if($security_roles['res']){
                        $i=1;
                        foreach($security_roles['rows'] as $security_role){
                            ?>
                    <tr>
                      <td><?=$i?>.</td>
                      <td><input type="checkbox" value="<?=$security_role->id?>" class="innercheckbox" name="id[]"></td>
                      <td><?=$security_role->role_name?></td>
                      <td><?=$security_role->role_description?></td>
                      <?php if($security_role->status=='Active') { ?>
                      <td><p style="color:blue"><?=$security_role->status?></p></td>
                      <?php } else { ?>
                      <td><p style="color:red"><?=$security_role->status?></p></td>
                      <?php }?>
                      <td style="width: 6px"><a href="<?=BASE_URL?>admin/security/edit_role/<?=$security_role->id?>" class="btn btn-warning btn-sm" title="Edit page"><span class="glyphicon glyphicon-pencil"></span></a></td>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Active Security Roles</h4>
      </div>
...
<div id="un_featured_user_model" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">In-Active Security Roles</h4>
      </div>
      <div class="modal-body">
    ...
    $(document).ready(function(){
        ...
        $(".featured").click(function(){
            if(!$(".innercheckbox").is(':checked')){
                $("#un_che_featured_user_deletemsg").html("Please select at least one Security Role.");
                $("#un_che_featured_user_model").modal("show");
            }else{
                 $("#featured_user_deletemsg").html("Do you want to active selected Security Role(s)?");
        ...
        $(document).on("click","#cnf2featured",function(){
            ...
            $.post("<?=BASE_URL?>admin/security/active_role",{selectedmail:selectedmail},function(data,status){
        ...
        $(".un_featured").click(function(){
            if(!$(".innercheckbox").is(':checked')){
                $("#un_che_un_featured_user_deletemsg").html("Please select at least one Security Role.");
                $("#un_che_un_featured_user_model").modal("show");
            }else{
                 $("#un_featured_user_deletemsg").html("Do you want to in-active selected Security Role(s)?");
        ...
        $(document).on("click","#cnf2un_featured",function(){
            ...
            //alert(selectedmail.length);
            if(selectedmail.length > 0){
            $.post("<?=BASE_URL?>admin/security/inactive_role",{selectedmail:selectedmail},function(data,status){
                var obj= $.parseJSON(data);
            if(obj.status){
