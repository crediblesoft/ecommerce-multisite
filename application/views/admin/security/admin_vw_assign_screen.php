              ...
              <table class="table table-bordered">
                <tbody>
                    ...
                    <tr>
                      ...
                      <td><input type="checkbox" value="<?=$screen->security_id?>" class="enablecheckbox" name="Enable[]" <?php if($screen->enable==1){echo "checked=checked";} ?>></td>
                      <td><input type="checkbox" value="<?=$screen->security_id?>" class="modifiablecheckbox" name="Modifiable[]" <?php if($screen->modifiable==1){echo "checked=checked";} ?>></td>
...
// Add an click listner to the Modifiable checkbox element
var idExample = document.querySelectorAll('input[name="Modifiable[]"]');

for(i=0; i<idExample.length; i++) {// Cycle through them
idExample[i].addEventListener('click', function(e){e.stopImmediatePropagation(); //alert("You Clicked me!"+this.value+" "+this.checked); 
 $.post("<?=BASE_URL?>admin/security/update_modifiable",{security_id:this.value,mod_value:this.checked},function(data,status){
                var obj= $.parseJSON(data);
            });
}
)}

// Add an click listner to the Enable checkbox element
var idEnabled = document.querySelectorAll('input[name="Enable[]"]');

for(i=0; i<idEnabled.length; i++) {// Cycle through them
idEnabled[i].addEventListener('click', function(e){e.stopImmediatePropagation(); //alert("You Clicked me!"+this.value+" "+this.checked); 
 $.post("<?=BASE_URL?>admin/security/update_enable",{security_id:this.value,mod_value:this.checked},function(data,status){
                var obj= $.parseJSON(data);
            });
}
)}
