...
<div class="form-group">
                     <label for="email">By Category:</label>
                      <select class="form-control" name="category" id="category">
                                    ...
                                    ?>
                                        <option value="<?=$category->id?>"<?php if($products['rows'][0]->id==$category->id){echo "selected";} ?>><?=$category->category?></option>
                                        <?php } } ?>
              ...
              <a href="<?=BASE_URL?>admin/subcategories/add/<?=$products['rows'][0]->id?>/<?=$products['rows'][0]->category?>" class="btn btn-success btn-sm" <?php if($this->session->userdata(ADMIN_SESS.'admin_type')!='1' && $this->session->userdata(ADMIN_SESS.'Manage Sub Category_modifiable')!='1'){echo 'disabled onClick="return false;"';}?>><span class="glyphicon glyphicon-plus-sign"></span> Add Sub Category</a>
              <a href="javascript:void(0)" class="btn btn-warning btn-sm <?php if($this->session->userdata(ADMIN_SESS.'admin_type')!='1' && $this->session->userdata(ADMIN_SESS.'Manage Sub Category_enable')!='1'){echo '" disabled ';}?> featured" id="">Active</a>
              <a href="javascript:void(0)" class="btn btn-primary btn-sm <?php if($this->session->userdata(ADMIN_SESS.'admin_type')!='1' && $this->session->userdata(ADMIN_SESS.'Manage Sub Category_enable')!='1'){echo '" disabled ';}?> un_featured" id="">In-Active</a>
              ...
              <table class="table table-bordered">
                <tbody>
                    ...
                    <tr>
                      ...
                      <td><?php echo substr($product->sub_description,0,50); if(count($product->sub_description)>50){echo "...";} ?></td>
                      <td><?=$product->noofproduct?></td>
                      <td><?=$product->sub_status?></td>
                      ...
                      <td style="width: 6px"><a href="<?=BASE_URL?>admin/subcategories/edit/<?=$product->sub_catid?>" class="btn btn-warning btn-sm" title="Edit category" <?php if($this->session->userdata(ADMIN_SESS.'admin_type')!='1' && $this->session->userdata(ADMIN_SESS.'Manage Sub Category_modifiable')!='1'){echo 'disabled onClick="return false;"';}?>><span class="glyphicon glyphicon-pencil"></span></a></td>
                    </tr>
...
<script> 
    ...
    $(document).ready(function(){
      ...
      $(document).on("change","#category",function(){
            var catidval=$("#category").val();
            //alert("Category "+catidval);
           window.location.href = '<?=BASE_URL?>admin/subcategories/getsubcategorylist'+'/'+catidval;
            //$.post("<?=BASE_URL?>admin/subcategories/getsubcategorylist",{catidval:catidval},function(data,status){
             //   var obj = jQuery.parseJSON(data);
            //alert("Data from getsubcategory "+obj);
           // });
      });
