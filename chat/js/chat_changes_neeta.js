...
function chatWith(chatuser,chatId=0) {
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle,chatId) {
	 //alert(chatboxtitle);
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			$.post(baseUrl+"chat.php?action=sendchat", {to: chatId, message: message} , function(data){
...
console.log(data);
		$.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
				...
				if ($("#chatbox_"+chatboxtitle).length <= 0) {
					createChatBox(chatboxtitle,2,1);
				}
